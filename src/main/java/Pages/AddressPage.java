...
public class AddressPage extends BaseStepMethod {
    ...
    public void set_To_Text_adress() throws InterruptedException {
        ...
        Faker sahteadres=new Faker();
        setTextElement(INPUT_ADDRESSPAGE_ADRES_BASLIGI,"Test Adresi");
        setTextElement(INPUT_ADDRESSPAGE_AD,sahteadres.name().firstName().toLowerCase());
        setTextElement(INPUT_ADDRESSPAGE_SOYAD,sahteadres.name().lastName().toLowerCase());
        setTextElement(INPUT_ADDRESSPAGE_CEP_TEL,sahteadres.phoneNumber().cellPhone());
        selectElementVisibleText(SELECT_ADDRESSPAGE_IL,"KOCAELİ");
        selectElementVisibleText(SELECT_ADDRESSPAGE_ILCE,"GEBZE");
        waitFor(2);
        selectElementVisibleText(SELECT_ADDRESSPAGE_MAH,"KİRAZPINAR MAH");
        setTextElement(INPUT_ADDRESSPAGE_ADRES,sahteadres.address().fullAddress());
        setTextElement(INPUT_ADDRESSPAGE_POSTA_CODE,sahteadres.address().zipCode());
